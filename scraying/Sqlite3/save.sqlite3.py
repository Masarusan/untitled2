...
#このスクリプトを何回実行する場合は削除する。
#citiesテーブルを作成する。
c.execute('DROP TABLE IF EXISTS cities')
#citiesテーブルを作成する。
c.execute('''
    CREATE TABLE cities(
    rank integer, city text,
    population integer
    )
    ''')
#execute()メソッドの第2引数にはSQL文のパラメーターのリストを指定できる。
#パラメーターで置き換える場所(プレースホルダー)は？で指定する。
c.execute('INSERT INTO cities VALUES(?, ?, ?)',(1,"上海",
                                                24150000))
...
c.execute('SELECT * FROM cities') #保存データを取得するSELECT文を実行する。
