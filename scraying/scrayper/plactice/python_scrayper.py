...
def main():
    """
    メインの処理。fetch().scraype(),save()3つの関数を予備だす。
    """
...
def scrape(html):
    ...
    books = []
    for partial_html in re.findall(r'<a itemprop="url".*?</ul>\s*</a>\
</li>',html, re.DOTALL):#/書籍のURLはitemprop="url"という属性を持つa要素をhref属性から取得する。
        url = re.search(r'<a itemprop="url" href="(.*?)">,\
                    partial_html').group(1)
...
def save(db_path, books):
    ...
    #インポートされたときに、main()関数が実行されないようにするための、Pythonに於ける一般的なイディオム
