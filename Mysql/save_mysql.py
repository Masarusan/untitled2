...
#ユーザー名とパスワードを指定してscrapingデータベースを使用する。接続に使用する文字コードはutf8mb4
conn = MySQLdb.connect(db='scraping', user='scraper', password='password',
                     charset='utf8mb4')
c = conn.cursor()# カーソルを取得する。
#execute()メソッドでSQL文を実行する。
#このスクリプトを何回実行しても同じ結果にするために、citiesテーブルが存在する場合は削除する。
